...
class Colors
{
     ...
     * @return int
     */
    public static function getColorCode ( $color, $options = [ ] )
    {
        $code = (int) $color;
        if ( is_string( $color ) ) {
    ...
    public static function getFgColorString ( $colorCode )
    {
        ...
        $codes = array_filter( array_merge( $options, [ "3{$color}" ] ) );
    ...
    {
        list( $color, $options ) = self::extractColorAndOptions( $colorCode );
        $codes = array_filter( array_merge( $options, [ "4{$color}" ] ) );
    ...
    private static function extractColorAndOptions ( $colorCode )
    {
        ...
        return [ self::$codes[ $colorCode ], $options ];
    }
