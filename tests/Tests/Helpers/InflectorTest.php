...
class InflectorTest extends TestCase
{
     ...
     * @param $plural
     * @param $expectedSingular
     */
    public function testSingularize($plural, $expectedSingular)
    {
        $singular = singular($plural);
        $this->implodeResult($expectedSingular, $singular);
     ...
     * @param $singular
     * @param $expectedPlural
     */
    public function testPluralize($singular, $expectedPlural)
    {
        $plural = plural($singular);
        $this->implodeResult($expectedPlural, $plural);
     ...
     * @param string $source
     */
    private function implodeResult($expected, string $source): void
    {
        if (\is_string($expected) && \is_array($source)) {
            $this->fail("--- Expected\n`string`: " . $expected . "\n+++ Actual\n`array`: " . implode(', ', $source));
        } elseif (\is_array($expected) && \is_string($source)) {
